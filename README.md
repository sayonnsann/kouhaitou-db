...
- 前場寄付モードは「当日(JST)分の始値がまだ無い」銘柄（薄商い・未寄り付き・
  取得失敗）を前回値のまま保持する。後場引けモードも同様に「当日(JST)分の
  終値がまだ無い」銘柄を前回値のまま保持する（土日祝など前営業日の終値しか
  無い場合は当日分として採用しない）。
- 1銘柄も取得できなかった場合はCSVを書き換えずwarningで終了する（Actionsは失敗にしない）。
  土日祝に後場引けジョブが走った場合も、全銘柄が「当日分なし」になりこの
  全滅パスに入るため、無意味なCSV更新・コミット・jsDelivr purgeは発生しない。
